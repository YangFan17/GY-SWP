...
@Component({
    moduleId: module.id,
    selector: 'out-scan-record',
    templateUrl: 'out-scan-record.component.html'
})
export class OutScanRecordComponent extends PagedListingComponentBase<any>{
    exportLoading = false;
    ...
    protected fetchDataList(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
        ...
        this.logisticService.getPagedOutStorageScanAsync(params)
            .finally(() => {
                finishedCallback();
            })
    ...
    export() {
        ...
        params.BeginTime = this.search.beginTime;
        params.EndTime = this.search.endTime;
        this.logisticService.exportOutScanRecord(params).subscribe((data => {
            if (data.code == 0) {
