        ...
        public async Task<APIResultDto> InitDeptmentCategory()
        {
            ...
            foreach (var item in list)
            {
                ...
                if (item != 1)
                {
                    ...
                    var category3Id = await _entityRepository.InsertAndGetIdAsync(category3);
                    await CurrentUnitOfWork.SaveChangesAsync();
                    var category301 = new Category();
                    category301.Name = "法律法规";
                    category301.DeptId = item;
                    category301.CreatorUserId = userId;
                    category301.CreationTime = dateNow;
                    category301.ParentId = category3Id;
                    category301.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category301);
                    var category302 = new Category();
                    category302.Name = "上级文件";
                    category302.DeptId = item;
                    category302.CreatorUserId = userId;
                    category302.CreationTime = dateNow;
                    category302.ParentId = category3Id;
                    category302.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category302);
                    var category303 = new Category();
                    category303.Name = "外来标准";
                    category303.DeptId = item;
                    category303.CreatorUserId = userId;
                    category303.CreationTime = dateNow;
                    category303.ParentId = category3Id;
                    category303.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category303);
                    ...
                    var category8Id = await _entityRepository.InsertAndGetIdAsync(category8);
                    await CurrentUnitOfWork.SaveChangesAsync();
                    var category801 = new Category();
                    category801.Name = "法律法规";
                    category801.DeptId = item;
                    category801.CreatorUserId = userId;
                    category801.CreationTime = dateNow;
                    category801.ParentId = category8Id;
                    category801.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category801);
                    var category802 = new Category();
                    category802.Name = "上级文件";
                    category802.DeptId = item;
                    category802.CreatorUserId = userId;
                    category802.CreationTime = dateNow;
                    category802.ParentId = category8Id;
                    category802.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category802);
                    var category803 = new Category();
                    category803.Name = "外来标准";
                    category803.DeptId = item;
                    category803.CreatorUserId = userId;
                    category803.CreationTime = dateNow;
                    category803.ParentId = category8Id;
                    category803.Desc = "[系统默认生成，无法修改]";
                    await _entityRepository.InsertAsync(category803);
